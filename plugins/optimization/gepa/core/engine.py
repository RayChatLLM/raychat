...
from ..logging.utils import log_detailed_metrics_after_discovering_new_program
from ..proposer.merge import MergeProposer
from ..proposer.reflective_mutation.reflective_mutation import (
    ReflectiveMutationProposer,
)
from ..strategies.eval_policy import EvaluationPolicy, FullEvaluationPolicy
from ..utils import StopperProtocol
...
class GEPAEngine(Generic[DataId, DataInst, Trajectory, RolloutOutput]):
    ...
    def __init__(
        ...
        merge_proposer: MergeProposer[DataId] | None,
        ...
        self.val_evaluation_policy: EvaluationPolicy = (
            val_evaluation_policy
            if val_evaluation_policy is not None
            else FullEvaluationPolicy()
        )
    ...
    def run(self) -> GEPAState[RolloutOutput, DataId]:
        ...
        valset = self.valset
        if valset is None:
            error_message = "valset must be provided to GEPAEngine.run()"
                ...
                if self.merge_proposer is not None and self.merge_proposer.use_merge:
                    if (
                        ...
                        if proposal is not None and proposal.tag == "merge":
                            ...
                            # Skip reflection after an accepted merge
                            continue
