...
from ..core.adapter import Candidate, DataInst, RolloutOutput
from ..core.data_loader import DataId, DataLoader
from ..core.state import GEPAState, ObjectiveScores, ProgramIdx
...
def filter_ancestors(
    ...
    for ancestor in common_ancestors:
        ...
        if not does_triplet_have_desirable_predictors(
            program_candidates,
            ancestor,
            i,
            j,
...
def find_common_ancestor_pair(
    ...
    for _ in range(max_attempts):
        ...
        common_ancestor_set = set(ancestors_i) & set(ancestors_j)
        common_ancestors = filter_ancestors(
            i,
            j,
            common_ancestor_set,
            merges_performed,
...
def sample_and_attempt_merge_programs_by_common_predictors(
    ...
    for _ in range(max_attempts):
        ...
        if (id1, id2, ancestor) in merges_performed[0]:
            continue
        assert agg_scores[ancestor] <= agg_scores[id1], (
            "Ancestor should not be better than its descendants"
        )
        assert agg_scores[ancestor] <= agg_scores[id2], (
            "Ancestor should not be better than its descendants"
        )
        ...
        assert (
            pred_names
            == set(program_candidates[id1].keys())
        ...
        for pred_name in pred_names:
            ...
            if (pred_anc in (pred_id1, pred_id2)) and pred_id1 != pred_id2:
            ...
            elif pred_anc not in (pred_id1, pred_id2):
                ...
                prog_to_get_instruction_from = (
                    ...
                    if agg_scores[id1] > agg_scores[id2]
                        ...
                        else rng.choice([id1, id2])
                    )
                )
                new_program[pred_name] = program_candidates[
                    prog_to_get_instruction_from
                ][pred_name]
                ...
                error_message = "Unexpected case in predictor merging logic"
...
class MergeProposer(ProposeNewCandidate[DataId]):
    ...
    def __init__(
        ...
        if val_overlap_floor <= 0:
            error_message = "val_overlap_floor should be a positive integer"
    ...
    def propose(
        ...
        if not (
            self.use_merge and self.last_iter_found_new_program and self.merges_due > 0
        ):
        ...
        assert set(subsample_ids).issubset(
